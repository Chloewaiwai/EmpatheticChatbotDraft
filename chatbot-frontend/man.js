class Chatbox {
    ...
    onSendButton(chatbox) {
        ...
        fetch('http://127.0.0.1:5000/predict', {
            method: 'POST',
            body: JSON.stringify({ message: text1 , bot:"John"}),
            mode: 'cors',
            headers: {
    ...
    async updateChatText(chatbox) {
        ...
        var html = '<div class="message"> <img src="man.png" alt="icon" class="icon"> <div class="messages__item messages__item--visitor">Hi! It\'s John! Long time no see! How\'s you going?</div> </div>';
        ...
        for (let message = 0;message< allMessage.length;message++){
            ...
            {   
                ...
                {
                    await delay(2500);
                    html = html.slice(0,-174)
                    html += '<div class="messageAnimate">  <img src="man.png" alt="icon" class="icon"> <div class="messages__item messages__item--visitor">' + allMessage[message].message + '</div></div>  </div>'
                }
                else
                {
                    html = html.slice(0,-174)
                    html += '<div class="message">  <img src="man.png" alt="icon" class="icon"> <div class="messages__item messages__item--visitor">' + allMessage[message].message + '</div></div>  </div>'
                }
            ...
            {   
                if (message===allMessage.length-2)
                {
                    ...
                    html += '<div class="threedots"><img src="man.png" alt="icon" class="icon"><div class="tiblock"><div class="tidot"></div><div class="tidot"></div><div class="tidot"></div></div></div>'
                    await delay(1000);
                    chatmessage.innerHTML = html;  
                    chatmessage.scrollTop=chatmessage.scrollHeight;
                }
